...
- [3. 라이브러리](#3-라이브러리)
  - [3.1. 스프링 부트 라이브러리](#31-스프링-부트-라이브러리)
  - [3.2. 테스트 라이브러리](#32-테스트-라이브러리)
- [4. 테스트 케이스 작성](#4-테스트-케이스-작성)
  - [4.1. 통합 테스트 케이스](#41-통합-테스트-케이스)
- [5. 스프링 Bean과 의존관계](#5-스프링-bean과-의존관계)
  - [5.1. 컴포넌트 스캔과 자동 의존관계](#51-컴포넌트-스캔과-자동-의존관계)
  - [5.2. 자바 코드로 직접 스프링 Bean 등록하기](#52-자바-코드로-직접-스프링-bean-등록하기)
- [6. 의존관계 주입 @Autowired](#6-의존관계-주입-autowired)
  - [6.1. 생성자 주입](#61-생성자-주입)
  - [6.2. 수정자 주입](#62-수정자-주입)
  - [6.3. 필드 주입](#63-필드-주입)
  - [6.4. 메서드 주입](#64-메서드-주입)
- [7. @Autowired의 동작](#7-autowired의-동작)
  - [7.1. Bean의 중복](#71-bean의-중복)
  - [7.2. 조회한 모든 Bean이 필요할 때](#72-조회한-모든-bean이-필요할-때)
  - [7.3. Bean이 없음](#73-bean이-없음)
- [8. 스프링 Bean 조회](#8-스프링-bean-조회)
  - [8.1. ApplicationContext](#81-applicationcontext)
  - [8.2. beanDefinition](#82-beandefinition)
- [9. Bean 생명주기Lifecycle 콜백callback](#9-bean-생명주기lifecycle-콜백callback)
  - [9.1. 스프링 Bean의 Lifecycle](#91-스프링-bean의-lifecycle)
  - [9.2. Lifecycle callback](#92-lifecycle-callback)
    - [9.2.1. InitalizingBean, DisposableBean 인터페이스](#921-initalizingbean-disposablebean-인터페이스)
    - [9.2.2. @Bean 설정에 메서드 지정](#922-bean-설정에-메서드-지정)
    - [9.2.3. 애노테이션 @PostConstruct, @PreDestroy 사용](#923-애노테이션-postconstruct-predestroy-사용)
- [10. Bean Scope](#10-bean-scope)
- [11. Web Scope](#11-web-scope)
  - [11.1. 환경 추가](#111-환경-추가)
  - [11.2. 포트 변경](#112-포트-변경)
  - [11.3. request](#113-request)
    - [11.3.1. 오류](#1131-오류)
    - [11.3.2. 해결방법](#1132-해결방법)
- [12. 롬복Lombok 라이브러리](#12-롬복lombok-라이브러리)
  - [12.1. 설정](#121-설정)
  - [12.2. 사용하기](#122-사용하기)
- [13. AOP (Aspect Oriented Programming)](#13-aop-aspect-oriented-programming)
- [14. View](#14-view)
  - [14.1. Welcome Page](#141-welcome-page)
  - [14.2. thymeleaf 템플릿](#142-thymeleaf-템플릿)
  - [14.3. 정적 컨텐츠](#143-정적-컨텐츠)
- [15. Controller](#15-controller)
  - [15.1. 템플릿 엔진 동작](#151-템플릿-엔진-동작)
- [16. 스프링 DB 접근 기술](#16-스프링-db-접근-기술)
  - [16.1. H2 DB 설치](#161-h2-db-설치)
  - [16.2. 순수 JDBC](#162-순수-jdbc)
- [17. 스프링 JDBC Template](#17-스프링-jdbc-template)
  - [17.1. insert](#171-insert)
  - [17.2. select](#172-select)
- [18. JPA(Java Persistence API)](#18-jpajava-persistence-api)
...
## 9. Bean 생명주기Lifecycle 콜백callback

### 9.1. 스프링 Bean의 Lifecycle

스프링 컨테이너 생성 -> 스프링 빈 생성 -> 의존관계 주입 -> 초기화 콜백 -> 사용 -> 소멸전 콜백 -> 스프링 종료

### 9.2. Lifecycle callback

#### 9.2.1. InitalizingBean, DisposableBean 인터페이스
...
#### 9.2.2. @Bean 설정에 메서드 지정

1. `@Bean(initMethod = "", destroyMethod = "")` 따옴표 사이에 메서드 이름 작성

> 외부 라이브러리에 사용

#### 9.2.3. 애노테이션 @PostConstruct, @PreDestroy 사용
